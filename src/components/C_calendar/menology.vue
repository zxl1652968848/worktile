<script src="../../../../123/worktile/src/main.js"></script>
<template>
    <div class="box">
        <div class="sun">
            <div class="s_header">
                <span>日历</span><i class="iconfont icon-rili"></i>
            </div>
            <div class="s_menlogy">
                <p class="s_m_title">
            ...
            <div class="myday">
                <p @click="mythingsshowfun"> <i class="iconfont icon-rili" >
                </i>我的日程<i class="iconfont icon-WIFI hou"></i></p>
                <transition name="fade">
                    <ul v-show="mythingsshow" transiton="fade">
                        <li v-for="item of mythings" @click="adddaythings(item)" ><i></i>{{item.title}}<b></b></li>
                    </ul>
                </transition>
                <p @click="teamthingsshowfun">
                    <i class="iconfont icon-sanjiaoyou" :class="{'rotate-caret':select}" @click="dian"></i>
                    团队日程
                    <i class="iconfont icon-more hou" ></i></p>
                <transition name="fade">
                    <ul v-show="teamthingsshow" transiton="fade">
                        <li v-for="item of teamthings" @click="adddaythings(item)"><i></i>{{item.title}}<b></b></li>
                    </ul>
                </transition>
                <p @click="peoplethingsshowfun"><i class="iconfont icon-sanjiaoyou" :class="{'rotate-caret':select1}" @click="dian2"></i>成员日程<i class="iconfont icon-jia hou"></i></p>
                <transition name="fade">
        ...
        <div class="newschedule_box" v-show="newscheduleboxshow" @scroll="woListScroll($event)">
            <div class="newschedule" :class="{opennewschedule : opennewshow}">
                ...
                <p>
                    <span>开始日期</span>
                    <button @click="leftsmallshow = !leftsmallshow" v-model="timestart">{{this.timestart}}</button>
                    <span>结束日期</span>
